...
def cp_file(srcpath, dstpath):
	...
	#更新lib文件
	cp_dir(srcpath + "dist\\lib", dstpath + "lib")	
	#更新DevConfig文件
	cp_dir(srcpath+ "DevConfig", dstpath+ "DevConfig")
...
def publish(srcpath, dstpath, config_file, info):
	#读取配置文件
	...
	#压缩内部版本
	cmd = "\"C:\\Program Files\\7-Zip\\7z.exe\" a " + dstpath.rstrip('\\') + info + "." + version + ".7z  " + dstpath 
	print(cmd)
	os.system(cmd)
...
def main():
	#源文件
	dst_path = ".\\output\\"
	src_path = "..\\spectra_nexus_windows\\"
	server_path = "Z:\\胖子\\光谱\\"

	#清空旧的打包文件
	clean_zip(dst_path)
	#复制需要的文件信息
	cp_file(src_path, dst_path + "SpectraNexus\\")
	#打包发布版本
	publish(src_path, dst_path + "SpectraNexus\\", "DevConfig\\SystemConfig.xml", "")
