...
//    private BaseChartTable baseChartandTable = new BaseChartTable();
    ...
    private void InitTableAndChart() {
        ...
        Text = dataChartPane.GetFreeTextPaint().CreateNewLable("");
    }

    //刷新主曲线
    public void UpdateMainData(double[] w, double[] v) {
        if (w.length == 0) {
            this.dataChartPane.DisplaySPData("", null);
            this.dataTablePane.UpdateData(null);
        } else {
            this.dataTablePane.UpdateData(new TSPData(w, v));
            this.dataChartPane.DisplaySPData("main", new CSPData(w, v));
        }
    }

    //刷新快照
    public void UpdataChartSnapShort(double w[], double []... vs) {
        String[] name = new String[vs.length];
        CSPData[] data = new CSPData[vs.length];
        for (int i = 0; i < name.length; i++) {
            name[i] = "SP-" + i;
            data[i] = new CSPData(w, vs[i]);
        }
